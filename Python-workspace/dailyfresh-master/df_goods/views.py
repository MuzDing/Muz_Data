...
def index(request):
    ...
    electronic = GoodsInfo.objects.filter(gtype__id=2).order_by("-id")[:4]
    electronic2 = GoodsInfo.objects.filter(gtype__id=2).order_by("-gclick")[:3]
    clothes = GoodsInfo.objects.filter(gtype__id=3).order_by("-id")[:4]
    clothes2 = GoodsInfo.objects.filter(gtype__id=3).order_by("-gclick")[:4]
    food = GoodsInfo.objects.filter(gtype__id=4).order_by("-id")[:4]
    food2 = GoodsInfo.objects.filter(gtype__id=4).order_by("-gclick")[:4]
    daily = GoodsInfo.objects.filter(gtype__id=5).order_by("-id")[:4]
    daily2 = GoodsInfo.objects.filter(gtype__id=5).order_by("-gclick")[:4]
    game = GoodsInfo.objects.filter(gtype__id=6).order_by("-id")[:4]
    game2 = GoodsInfo.objects.filter(gtype__id=6).order_by("-gclick")[:4]
    # count = CartInfo.objects.filter(
    #     user_id=request.session.get('userid')).count()   'count':count,
    # # 构造上下文
    context = {'title': '首页', 'fruit': fruit,
               'electronic': electronic, 'clothes': clothes, 'food': food2,
               ' daily':  daily, 'game': game,
               'fruit2': fruit2, 'electronic2': electronic2, 'clothes2': clothes,
               'food2': food2, ' daily2':  daily2, 'game2': game2,
